...
/**
 * Admin attachment download proxy. The browser cannot attach the Bearer token,
 * so this handler asks the Go API for the attachment and forwards the browser to it.
 *
 * Real upstream contract (backend/internal/modules/tickets/handler.go):
 *   GET /api/v1/admin/tickets/:id/attachments/:idx  (idx = position over the
 *   visible thread's attachments, as returned in each reply's
 *   AttachmentView.index - NOT a query-string object key, despite
 *   docs/RECONCILE.md's earlier guess).
...
export const GET: RequestHandler = async (event) => {
	...
	const upstreamUrl = new URL(
		`${apiUrl()}/api/v1/admin/tickets/${event.params.id}/attachments/${event.params.idx}`
	);

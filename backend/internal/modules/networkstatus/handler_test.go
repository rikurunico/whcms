...
// fakeMW stubs networkstatus.Middlewares: injects a fixed identity and mirrors
...
var _ networkstatus.Middlewares = (*fakeMW)(nil)
...
func newApp(mut func(*fakeMW)) *harness {
	...
	h := networkstatus.NewHandler(fx.svc, mw)
...
func TestHandlerPublicList(t *testing.T) {
	h := newApp(nil)
	h.fx.repo.ListActiveFn = func(ctx context.Context) ([]domain.NetworkIssue, error) {
		return []domain.NetworkIssue{{ID: 1, Title: "Outage", Status: domain.NetworkStatusInvestigating}}, nil
	}
	resp, env := h.do(t, "GET", "/api/v1/network-status", nil)
	assert.Equal(t, 200, resp.StatusCode)
	require.Nil(t, env.Error)
	require.NotNil(t, env.Data)
	assert.Equal(t, 1, h.mw.rateLimited, "public list route is rate limited")
}

func TestHandlerPublicGet(t *testing.T) {
	h := newApp(nil)
	h.fx.repo.GetByIDFn = func(ctx context.Context, id int64) (*domain.NetworkIssue, error) {
		return &domain.NetworkIssue{ID: id, Title: "Outage", Status: domain.NetworkStatusInvestigating}, nil
	}
	resp, env := h.do(t, "GET", "/api/v1/network-status/5", nil)
	assert.Equal(t, 200, resp.StatusCode)
	require.Nil(t, env.Error)
	raw, _ := json.Marshal(env.Data)
	var got domain.NetworkIssue
	require.NoError(t, json.Unmarshal(raw, &got))
	assert.Equal(t, int64(5), got.ID)
	assert.Equal(t, 1, h.mw.rateLimited, "public detail route is rate limited")
}

func TestHandlerPublicGetNotFound(t *testing.T) {
	h := newApp(nil)
	// Default GetByID mock returns nil,nil -> service maps to NOT_FOUND.
	resp, env := h.do(t, "GET", "/api/v1/network-status/999", nil)
	assert.Equal(t, 404, resp.StatusCode)
	require.NotNil(t, env.Error)
	assert.Equal(t, "NOT_FOUND", env.Error.Code)
}

func TestHandlerPublicGetInvalidID(t *testing.T) {
	h := newApp(nil)
	resp, env := h.do(t, "GET", "/api/v1/network-status/abc", nil)
...
func TestHandlerAdminRequiresAuth(t *testing.T) {
	...
	resp, env := h.do(t, "GET", "/api/v1/admin/network-issues", nil)
...
func TestHandlerAdminStaffWithPermission(t *testing.T) {
	h := newApp(func(m *fakeMW) {
		m.identity = httpx.AuthIdentity{UserID: 5, Role: "staff"}
		m.permissions = map[string]bool{"network": true}
	})
	resp, _ := h.do(t, "GET", "/api/v1/admin/network-issues", nil)
	assert.Equal(t, 200, resp.StatusCode)
}
...
func TestHandlerAdminList(t *testing.T) {
	h := newApp(nil)
	h.fx.repo.ListFn = func(ctx context.Context, p ports.ListParams) ([]domain.NetworkIssue, int64, error) {
		assert.Equal(t, "outage", p.Search)
		assert.Equal(t, "investigating", p.Status)
		return []domain.NetworkIssue{{ID: 1}}, 1, nil
	}
	resp, env := h.do(t, "GET",
		"/api/v1/admin/network-issues?search=outage&status=investigating&page=2&per_page=10", nil)
	assert.Equal(t, 200, resp.StatusCode)
...
func TestHandlerAdminCreate(t *testing.T) {
	h := newApp(nil)
	h.fx.repo.CreateFn = func(ctx context.Context, n *domain.NetworkIssue) error {
		n.ID = 3
		return nil
	}
	resp, env := h.do(t, "POST", "/api/v1/admin/network-issues", map[string]any{
		"title": "DB outage", "body": "details", "type": "outage", "severity": "major",
	})
...
func TestHandlerAdminCreateInvalidBody(t *testing.T) {
	h := newApp(nil)
	resp, env := h.do(t, "POST", "/api/v1/admin/network-issues", map[string]any{"title": "x"})
	assert.Equal(t, 422, resp.StatusCode)
	assert.Equal(t, "VALIDATION", env.Error.Code)
}
...
func TestHandlerAdminGet(t *testing.T) {
	h := newApp(nil)
	h.fx.repo.GetByIDFn = func(ctx context.Context, id int64) (*domain.NetworkIssue, error) {
		return &domain.NetworkIssue{ID: id, Title: "Hello"}, nil
	}
	resp, env := h.do(t, "GET", "/api/v1/admin/network-issues/7", nil)
	assert.Equal(t, 200, resp.StatusCode)
	require.Nil(t, env.Error)
}
...
func TestHandlerAdminUpdate(t *testing.T) {
	...
	h.fx.repo.GetByIDFn = func(ctx context.Context, id int64) (*domain.NetworkIssue, error) {
		return &domain.NetworkIssue{
			ID: id, Title: "Old", Type: domain.NetworkTypeIssue,
			Severity: domain.NetworkSeverityMinor, Status: domain.NetworkStatusInvestigating,
		}, nil
	}
	resp, env := h.do(t, "PATCH", "/api/v1/admin/network-issues/7", map[string]any{"title": "New Title"})
	assert.Equal(t, 200, resp.StatusCode)
	require.Nil(t, env.Error)
}
...
func TestHandlerAdminDelete(t *testing.T) {
	h := newApp(nil)
	h.fx.repo.GetByIDFn = func(ctx context.Context, id int64) (*domain.NetworkIssue, error) {
		return &domain.NetworkIssue{ID: id}, nil
	}
	resp, _ := h.do(t, "DELETE", "/api/v1/admin/network-issues/7", nil)
	assert.Equal(t, 204, resp.StatusCode)
}

func TestHandlerAdminDeleteNotFound(t *testing.T) {
	h := newApp(nil)
	resp, env := h.do(t, "DELETE", "/api/v1/admin/network-issues/7", nil)
	assert.Equal(t, 404, resp.StatusCode)
	assert.Equal(t, "NOT_FOUND", env.Error.Code)
}

func TestHandlerAdminInvalidID(t *testing.T) {
	h := newApp(nil)
	resp, env := h.do(t, "GET", "/api/v1/admin/network-issues/abc", nil)
	assert.Equal(t, 422, resp.StatusCode)
	assert.Equal(t, "VALIDATION", env.Error.Code)
}
...
func TestHandlerAdminInvalidJSONBodySweep(t *testing.T) {
	tests := []struct {
		...
		{"create", "POST", "/api/v1/admin/network-issues"},
		{"update", "PATCH", "/api/v1/admin/network-issues/1"},
	}
...
// Invalid :id params: every parseID-guarded endpoint (public + admin) must
// reject a non-numeric id with 422 VALIDATION before reaching the service.

func TestHandlerInvalidIDSweep(t *testing.T) {
	tests := []struct {
		name   string
		method string
		path   string
	}{
		{"public get", "GET", "/api/v1/network-status/abc"},
		{"admin get", "GET", "/api/v1/admin/network-issues/abc"},
		{"admin update", "PATCH", "/api/v1/admin/network-issues/abc"},
		{"admin delete", "DELETE", "/api/v1/admin/network-issues/abc"},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			h := newApp(nil)
...
func TestHandlerServiceErrorSweep(t *testing.T) {
	tests := []struct {
		...
		{"public list", "GET", "/api/v1/network-status", nil, func(fx *fixtures) {
			fx.repo.ListActiveFn = func(context.Context) ([]domain.NetworkIssue, error) { return nil, errBoom }
		}},
		{"public get", "GET", "/api/v1/network-status/7", nil, func(fx *fixtures) {
			fx.repo.GetByIDFn = func(context.Context, int64) (*domain.NetworkIssue, error) { return nil, errBoom }
		}},
		{"admin list", "GET", "/api/v1/admin/network-issues", nil, func(fx *fixtures) {
			fx.repo.ListFn = func(context.Context, ports.ListParams) ([]domain.NetworkIssue, int64, error) {
				return nil, 0, errBoom
			}
		}},
		{"admin create", "POST", "/api/v1/admin/network-issues",
			map[string]any{"title": "A valid title"}, func(fx *fixtures) {
				fx.repo.CreateFn = func(context.Context, *domain.NetworkIssue) error { return errBoom }
			}},
		{"admin get", "GET", "/api/v1/admin/network-issues/7", nil, func(fx *fixtures) {
			fx.repo.GetByIDFn = func(context.Context, int64) (*domain.NetworkIssue, error) { return nil, errBoom }
		}},
		{"admin update", "PATCH", "/api/v1/admin/network-issues/7",
			map[string]any{"title": "A valid title"}, func(fx *fixtures) {
				fx.repo.GetByIDFn = func(context.Context, int64) (*domain.NetworkIssue, error) { return nil, errBoom }
			}},
		{"admin delete", "DELETE", "/api/v1/admin/network-issues/7", nil, func(fx *fixtures) {
			fx.repo.GetByIDFn = func(context.Context, int64) (*domain.NetworkIssue, error) { return nil, errBoom }
		}},
	}
	for _, tt := range tests {

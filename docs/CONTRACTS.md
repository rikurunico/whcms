...
- catalog (public): `GET /products`, `GET /products/:slug`, `GET /product-groups`, `POST /domains/check` {names[]}, `POST /coupons/validate`.
...
- payments: `GET /payments/methods?invoice_id=`, `POST /webhooks/duitku`, `GET /payments/return`; admin: `GET /admin/transactions?search&status&gateway&page&per_page`, `POST /admin/transactions/:id/confirm`; gateways: `GET/PUT /admin/gateways` (duitku), `PUT /admin/gateways/manual`.
